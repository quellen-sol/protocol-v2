...
describe('emit user trading summary', () => {
	...
	it('emits user trading summary', async () => {
		await driftClient.emitUserTradingSummary(userAccountPk);

		for (const event of eventSubscriber.getEventList(
			'UserTradingSummaryRecord'
		)) {
			assert(event.authority.equals(driftClient.authority));
			assert(event.open_order_count == 1);
		}
	});
});
